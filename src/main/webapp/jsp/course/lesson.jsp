...
<title>${userCourse.course.courseTitle}-萌课网</title>
...
<c:if test="${empty user.userId}">
  ...
  <div class="flat">
	...
	<c:if test="${user.userId!=userCourse.user.userId}">				    
      ...
      <div  class="nickname"><a href="" class="show-user-card" data-uid="1295165" title="${userCourse.user.userName}">${userCourse.user.userName}<span class="o-ver-icn" title="${userCourse.user.intro}"></span></a></div>
      <div>
        <a href="/u/tsingyuan/library?tab=created" class="stats"><em>4</em> 课程</a>
        <a href="/u/tsingyuan/fans" class="stats">
          <em>${fansNum}</em> 粉丝</a>
      </div>
        <div class="mtm gray">${userCourse.user.intro}</div>
    </div>  
			 ...
			 <div class="flat">
                 ...
                 <a href="" title="${uc3.user.userName}"><img src="${uc3.user.headImage.imageMid}"  alt="${uc3.user.userName}的头像" width=24 height=24></a>
                 </c:forEach>
                 </c:if>            
             </div>
    ...
    <div class="flat">
     ...
     <c:if test="${leaning>0}">
      <h3>${leaning}人正在学习</h3>
         ...
         <a href="" title="${uc2.user.userName}"><img src="${uc2.user.headImage.imageMid}"  alt="${uc2.user.userName}的头像" width=24 height=24></a>
         </c:forEach>
      </c:if>            
   </div>
